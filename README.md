...
## 🎯 Бизнес-суть проекта (Контекст)

Этот проект представляет собой **полностью автоматизированную воронку для HR-найма курьеров** (Яндекс Еда, Яндекс Курьеры и др.) через Telegram. 
Система решает задачу массового поиска, привлечения и первичной обработки кандидатов без участия человека, закрывая весь цикл — от поиска лида до передачи готового кандидата живому HR-специалисту.

Два главных компонента системы:
1. **TG Combiner (Поиск и холодный контакт):** Автоматически парсит Telegram-чаты и группы. С помощью встроенного ИИ (Claude Opus) анализирует сообщения, фильтрует пользователей по курьерскому сленгу, возрасту и локации, а затем делает по подходящим кандидатам умные рассылки с приглашением на работу.
2. **CRM System (Обработка откликов):** Перехватывает ответы заинтересованных кандидатов через Telegram Business API. Встроенный автономный ИИ-рекрутер (DeepSeek) ведет с лидом естественный диалог, имитирует скорость печати человека, отвечает на вопросы об условиях работы и зарплате. Как только лид соглашается, ИИ ставит тег `[LEAD_READY]` и передает диалог живому оператору для оформления.
